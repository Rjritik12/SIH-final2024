...
TO_PHONE_NUMBER = "official_phone_number"
...
def send_sms_alert(body):
    ...
    print(f"SMS sent to {TO_PHONE_NUMBER}: {body}")

def send_real_time_alert_to_dashboard(message):
    socketio.emit('real_time_alert', {'message': message})
            ...
            try:
                ...
                if result["verified"]:
                    ...
                    if is_within_geofence(*CAMERA_LOCATION):
                        ...
                        send_sms_alert(f"Missing person found: Unknown at {CAMERA_LOCATION}")
                        send_real_time_alert_to_dashboard(f"Missing Person Found at {CAMERA_LOCATION}")
            ...
            try:
                ...
                if result["verified"]:
                    ...
                    if is_within_geofence(*CAMERA_LOCATION):
                        ...
                        send_sms_alert(f"Criminal Detected: {criminal_name} at {CAMERA_LOCATION}")
                        send_real_time_alert_to_dashboard(f"Criminal Detected: {criminal_name} at {CAMERA_LOCATION}")
            except Exception as e:
